...
instance Bind (State s) where
  (=<<) ::
    ...
    -> State s b
  f =<< State toV =
    State (\sv -> let (av, sv') = toV sv in runState (f av) sv')
...
findM p (x:.xs) = p x >>= (\res -> if res then return $ Full x else findM p xs)
...
firstRepeat ::
  ...
  List a
  -> Optional a
firstRepeat xs = eval (findM alreadyExists xs) S.empty
  where
    alreadyExists x = (\s -> (const $ pure (S.member x s)) =<< put (S.insert x s)) =<< get
...
distinct ::
  ...
  List a
  -> List a
-- State (S.Set a) (List a)
distinct xs = eval (filtering f xs) S.empty
  where
    f x = State (\set -> (S.notMember x set, S.insert x set))
...
isHappy ::
  Integer
  -> Bool
isHappy = contains 1 . firstRepeat . (produce sumOfSquares)
  where
    sumOfSquares = toInteger . sum . map (join (*) . digitToInt) . show'
