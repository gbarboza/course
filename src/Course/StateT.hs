...
instance Functor f => Functor (StateT s f) where
  (<$>) ::
    ...
    -> StateT s f a
    -> StateT s f b
  f <$> StateT k = StateT $ (\h -> (\(a', s') -> (f a', s')) <$> h) . k
...
instance Bind f => Apply (StateT s f) where
  ...
  sf <*> sk = StateT $ \s ->
    let
      ff = runStateT sf s
    in
      ff >>= \(f, s') -> (\(a, s'') -> (f a, s'')) <$> runStateT sk s'
...
instance Monad f => Bind (StateT s f) where
  (=<<) ::
    ...
    -> StateT s f a
    -> StateT s f b
  f =<< sx = StateT (\s -> (\(a, s') -> runStateT (f a) s') =<< runStateT sx s)
...
exec' ::
  ...
  -> s
  -> s
exec' st s = snd $ runId $ runStateT st s
...
eval' ::
  ...
  -> s
  -> a
eval' st s = fst $ runId $ runStateT st s
